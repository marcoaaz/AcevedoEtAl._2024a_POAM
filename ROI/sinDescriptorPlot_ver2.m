function sinDescriptorPlot_ver2(f, s, xData, yData, optical_period)
%sf = cfit of Fourier-2
%s = structure describing sf

%period = modality sexagesimal period (ppl = 180, xpl=90)
x_plot = 1:1:2*optical_period; %depends on data acquisition
...
%functions
f_output = double(subs(f, x_plot));
h2 = plot(f_output, 'LineWidth', 2*lineWidth, ...
...
h11 = plot([optical_period, optical_period], ylim(ax), 'LineWidth', lineWidth, 'Color', 'black', 'LineStyle','--', ...
    'DisplayName', sprintf('period = %0.1f', optical_period)); %period
