...
file_output2 = strcat(descripTxt{uu}, '_modulationImage_', ...
...
Imax_vector = [125; 125]; %max intensity found in [3D universal stage or 2D section]

%Configuring 

%mylonite quartz
filter_expression = "([stats_add.foreground]' == 1 & [stats_add.Solidity]' > .35 & [stats_add.Circularity]' > .15)";
%default: "([stats_add.foreground]' == 1)";
...
switch mappingAlgorithm
    ...
    case 4 % Algorithm 4
        ...
        importType = 2;
        [img_temp2_channels, ~] = stackImportLoop(imageName, info_struct, varRange, importType, scale);   
        ...
        [pixel_maxPeakI_discrete1] = largestPeak_channels(...
            img_temp2_channels, pol_angle1, pixel_maxPeak1); %closest discrete value
        ...
        if isfile(file_closest1) && isfile(file_closest2)
            ...
            t_finding_px = n_px_ds/time_chart; %56K px/sec 
